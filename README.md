...
我们用 K-Design 2026 的 1,612 件参赛作品检验 Skill 的实际筛选能力。所有作品都在最终获奖结果公布前完成评分，评分时无法知道哪些作品会获奖。

在同样选取前 115 名的条件下，Skill 命中 45 件获奖作品，GPT-5.4 直接评分命中 33 件。Skill 多命中 12 件，说明加入成熟度判断和统一排序规则后，比直接使用模型总分更适合筛选优先作品。该结果用于验证筛选方法，不代表单件作品一定获奖。[查看完整实测说明](docs/kdesign-2026-prediction-layer-results.md)
