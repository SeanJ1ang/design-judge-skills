...
Design Judge Skill 比 GPT-5.4 直接评分多命中 12 件。Skill 保留原始作品评分，再依据已确认的成熟度调整候选顺序。结果公示后只用已保存的分数核对获奖名单，没有重新评价作品，也没有修改原始分数。

这项实测说明，Skill 比只看模型总分更适合筛选优先项目，但不能保证单件作品获奖。仓库不公开私有数据库、作品图片、逐件评价、提示词、环境变量或 API 密钥。
